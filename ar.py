...
fitted = result.predict(start=1, end=len(full_df))
series = pd.Series(fitted, index=full_df.index)

model4 = AutoReg(df['Value'], lags=4)
result4 = model4.fit()
print(result4.summary())

fitted4 = result4.predict(start=4, end=len(full_df))
series4 = pd.Series(fitted4, index=full_df.index)

# y_true — ваш реальный ряд значений
y_true = full_df['Value']

# y_pred для модели с lags=1
y_pred1 = series  # предсказания result.predict(start=1, end=len(df))

# y_pred для модели с lags=4
y_pred4 = series4  # предсказания result4.predict(start=4, end=len(df))

# Рассчитываем
mae1, rmse1, mape1 = calc_metrics(y_true.tail(3), y_pred1.tail(3))
mae4, rmse4, mape4 = calc_metrics(y_true.tail(3), y_pred4.tail(3))

print("AutoReg(lags=1):")
print(f" MAE  = {mae1:.4f}")
print(f" RMSE = {rmse1:.4f}")
print(f" MAPE = {mape1:.2f}%\n")

print("AutoReg(lags=4):")
print(f" MAE  = {mae4:.4f}")
...
plt.plot(series.index,  series.values,  marker='o', label='AR(1)')
plt.plot(series4.index, series4.values, marker='o', label='AR(4)')
plt.legend()
plt.show()
input()