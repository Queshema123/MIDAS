...
class BetaWeights(Weights):
    ...
    def __init__(self, thetas:np.array, x:pd.DataFrame):
        '''
            theta1, theta2 - параметры тета. Количество 2 оптимально и позволяет сократить размерность.
            x - датафрейм с колонками Date и Value
        '''
        super().__init__(thetas, x)
    ...
    def mse_loss(self, th:np.array, start:pd.DatetimeIndex, end:pd.DatetimeIndex, true_values:np.array, forecast_vals:np.array):
        ...
        return np.mean((true_values-summary)**2)
...
class AlmonExpPolyWeights(Weights):
    def __init__(self, thetas: np.array, df: pd.DataFrame):
        super().__init__(thetas, df)
...
class AlmonPolynomialWeights(Weights):
    ...
    def mse_loss(self, th: np.array, start, end, true_vals, forecast_vals):
        self.params = th
        smoothed = forecast_vals + self.calc_sum(start, end)
        return np.mean((true_vals - smoothed) ** 2)
    ...
    def optimize_theta(self, start, end, true_vals, forecast_vals):
        res = minimize(
            ...
            bounds=[(None, None)] * len(self.params)
        )
        self.params = res.x
